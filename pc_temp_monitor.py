    ...
    if daemon_mode:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Не удалось настроить файловое логирование, используется консоль")
...
def get_cpu_temperature_linux(logger):
    ...
    if temp is None:
        logger.warning(
            "Не удалось получить температуру, используем безопасное значение 40°C")
        ...
        try:
            if os.path.exists(temp_path):
                with open(temp_path, 'r') as f:
                    ...
                    if 0 <= temp <= 120:
                        print(
                            f"AMD специфичный путь: {temp_path} = {
                                temp:.1f}°C")
                        return temp
                ...
                for temp_file in temp_files:
                    try:
                        with open(temp_file, 'r') as f:
                            temp_str = f.read().strip()
                            if not temp_str or not temp_str.replace(
                                    '.', '').isdigit():
                                continue
                            ...
                            if 0 <= temp <= 120:
                                ...
                                base_name = os.path.basename(
                                    temp_file).replace('_input', '')
                                label_file = os.path.join(
                                    hwmon_dir, f'{base_name}_label')
                                if os.path.exists(label_file):
                                    with open(label_file, 'r') as lf:
                                        label = lf.read().strip()
                                        print(
                                            f"hwmon: {device_name} - {label} = {temp:.1f}°C")
                                else:
                                    print(
                                        f"hwmon: {device_name} - {base_name} = {temp:.1f}°C")
                    except (ValueError, OSError, IOError):
                        continue
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if temps:
                ...
                print(
                    f"lm-sensors: Средняя температура CPU = {avg_temp:.1f}°C")
                return avg_temp
...
def check_temperature_sources_linux():
    ...
    print("-" * 60)
    print(
        f"Доступные источники температуры: {
            ', '.join(available_sources) if available_sources else 'Нет'}")
    ...
    if not available_sources:
        ...
        print(
            f"\nБудет использован первый доступный источник: {
                available_sources[0]}")
            ...
            while self.running:
                try:
                    ...
                    if cpu_temp is None or cpu_load is None:
                        self.logger.warning(
                            "Некорректные данные, повтор через 2 секунды")
                        time.sleep(2)
                        continue
                    ...
                    if self.send_data(data_str):
                        ...
                        if not self.daemon_mode or success_counter % 10 == 0:
                            self.logger.info(
                                f"CPU: {cpu_temp}°C, Load: {cpu_load}%")
                        ...
                        self.error_count += 1
                        self.logger.warning(
                            f"Ошибка отправки ({self.error_count}/{self.max_errors})")

                        if self.error_count >= self.max_errors:
                            self.logger.error(
                                "Превышено максимальное количество ошибок")
                            break
...
"""
    ...
    service_file = f"/etc/systemd/system/pc-temp-monitor-{
        os.getenv('USER')}.service"
    ...
    try:
        ...
        print(
            f"  sudo systemctl enable pc-temp-monitor-{os.getenv('USER')}.service")
        print(
            f"  sudo systemctl start pc-temp-monitor-{os.getenv('USER')}.service")
        print(
            f"  sudo systemctl status pc-temp-monitor-{os.getenv('USER')}.service")
        print(
            f"  sudo journalctl -u pc-temp-monitor-{os.getenv('USER')}.service -f")
...
Примеры использования:
  ...
  python3 %(prog)s --create-service --port /dev/ttyUSB0  # Создать systemd сервис
    ...
    parser.add_argument(
        '--port',
        help='COM порт ESP32 (например: /dev/ttyUSB0)')
    parser.add_argument(
        '--daemon',
        action='store_true',
    ...
    parser.add_argument(
        '--check-sensors',
        action='store_true',
        help='Проверить датчики температуры')
    parser.add_argument(
        '--list-ports',
        action='store_true',
        help='Показать доступные порты')
    parser.add_argument(
        '--log',
        action='store_true',
        help='Показать логи демона')
    parser.add_argument(
        '--create-service',
        action='store_true',
        ...
        if ports:
            ...
            for i, port in enumerate(ports, 1):
                try:
                    detected_ports = serial.tools.list_ports.comports()
                    desc = next(
                        (d for p, d, h in detected_ports if p == port), "")
                    if desc:
    ...
    if args.daemon:
        ...
        logger, log_location = setup_logging(
            daemon_mode=True, log_level=log_level)

        monitor = TemperatureMonitor(
            args.port, logger, log_location, daemon_mode=True)
        ...
        logger, log_location = setup_logging(
            daemon_mode=False, log_level=log_level)
        monitor = TemperatureMonitor(
            args.port, logger, log_location, daemon_mode=False)
